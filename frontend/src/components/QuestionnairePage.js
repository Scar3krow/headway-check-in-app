...
const QuestionnairePage = () => {
    ...
    const fetchQuestions = async () => {
      try {
        ...
        const response = await fetch(
          `${API_URL}/questions?questionnaire_id=${selectedQuestionnaire}`,
          {
            headers: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/user-data/${userId}/sessions/${sessionId}/responses`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="questionnaire-container">
      {isLoading && <LoadingMessage text="Loading questionnaire..." />}
        ...
        {questions.length > 0 && (
          <>
                ...
                (label, index) => (
                  <button
                    ...
                    className={`response-btn ${
                      responses[questions[currentIndex].id] === index + 1 ? "selected" : ""
                    }`}
                    onClick={() => {
                      handleAnswerSelect(questions[currentIndex].id, index + 1);
            ...
            {currentIndex === questions.length - 1 && (
              <button type="button" onClick={handleSubmit} className="submit-btn visible">
                Submit Responses
              </button>
            )}
          </>
